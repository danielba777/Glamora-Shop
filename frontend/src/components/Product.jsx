  ...
  return (
    <div className='bg-white p-4 rounded shadow-md mb-2 hover:border' key={product.productDisplayName}>
        <Link to={`/product/${product._id}`}>
            <img src={product.imageId == '123' ? `${product.image}` : `${process.env.PUBLIC_URL}/images/${product.imageId}.jpg`} alt={product.productDisplayName} className='w-full h-auto mb-4'/>
        </Link>
        <Link to={`/product/${product._id}`}>
